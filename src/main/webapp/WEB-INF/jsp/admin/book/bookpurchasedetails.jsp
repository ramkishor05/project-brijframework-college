				...
				if (length == 0) {
					...
					if(j[0].paymentMode=='Cheque')
	                   ...
	                   st = st
							...
							+ '<table class="table table-bordered table-primary"><thead><tr><th>S No</th><th>Class</th><th>Section</th><th>Subject</th><th>Title</th><th>Price</th><th>Quantity</th><th>Amount</th><th>Net Amount</th></tr></thead><tbody>';
					for ( var i = 0; i < length; i++) {
						st += '<tr><td>' + (i + 1)
								+ '</td><td>'+ j[i].className
								+ '</td><td>'+ j[i].sectionName
								+ '</td><td>' + j[i].subjectName
								+ '</td><td>' + j[i].bookTitle
								+ '</td><td>'+ j[i].bookPrice
...
function changePaidStatus(receiptno,status)
{
	...
	document.location.href='change-paidbook-status?id='+receiptno+'&status='+status;
	}

</script>
<div class="detailbox" style="width: 686px;">
	<div class="detailbox-header" style="width: 687px;">
...
<div class="content">
	<div id="content-header" style="height: 140px;">
		...
		<h3>View Purchased Book Details</h3>
		...
		<div class="block">
			...
			<div class="data-fluid">
				<table class="table fTable lcnp" style="width: 1059px;">
					...
					<tbody>
						<c:forEach var="list" items="${allbooks}" varStatus="loop">
							<tr>
								<td>${loop.count}</td>
							...
							<c:otherwise><td>${list.paidStatus}</td></c:otherwise>
								...
								<c:otherwise>
								<td><a href="viewpurchasedbook-receipt?id=${list.receiptNo}"target="_blank"><span
