...
<script>
	function defaulterView() {
...

		{
			...
			checklastdate(monthId);

		}

	}
	function checklastdate(monthId) {
		var sessionId = $("#sessionId").val();
		var classId = $("#classId").val();
		var sectionId = $("#sectionId").val();
		var monthId = $("#monthId").val();
		$("#DefaulterList").empty();
		var url = 'checklastdate/' + monthId;
		$
				.ajax({
					url : url,
					...
					success : function(data, status) {
						...
						var i = 0;
						if (j[i] == 1) {
							getDefaulterList(sessionId, classId, sectionId,
									monthId);
						} else {

							var st = '<div class="alert alert-success"><h5 align="center"><font color="red"><b>There are no defaulter students for this month and due date '
									+ j[i + 1] + '</b></font></h5></div>';
	...
	function getDefaulterList(sessionId, classId, sectionId, monthId) {

		var url = 'getDefaulterList/' + sessionId + '/' + classId + '/'
				+ sectionId + '/' + monthId;
		$
				.ajax({
					url : url,
					...
					success : function(data, status) {
						...
						var length = j.length;
					if (length == 0) {
						st = '<div class="alert alert-success"><h5 align="center"><font color="red"><b>There are no defaulter students</b></font></h5></div>';
					} else {
						st = st
						+ '<div><div class="widget-body" style="padding: 10px 0 0;"><div class="separator"></div><div class="btn-group" style="width: 400px;">'
						+ '</div><div class="block"><div class="head blue"><h4 style="text-align:center">Fee Defaulters List</h4></div>'
						+ '<table class="table table-bordered table-primary"><thead><tr><th>S.no</th><th>Student Name</th><th>Balance Amount</th><th>Mobile</th><th>Address</th></tr></thead><tbody>';
	...
	function getsection(value) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				var content = '<option value="">Select sections ...</option>';
...
<div class="content">
	<div id="content-header" style="height: 150px;">
		...
		<h3>Check MonthWise Fee Defaulter list</h3>
	...
	<f:form method="post" target="_blank" action="show-defaulter-pdf-report"
		...
		<tr>
			...
			<td><select id="classId" onchange="getsection(this.value);"
