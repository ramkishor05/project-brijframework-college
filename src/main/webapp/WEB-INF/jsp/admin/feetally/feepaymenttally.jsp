					...
					.ajax({
						...
						success : function(data, status) {
							...
							var studentDetails = '<div style="text-align: center; height: 33px; background-color: rgb(238, 238, 238); color: rgb(0, 136, 204); border: 1px solid lightgray; border-top-left-radius: 10px; border-top-right-radius: 10px; margin-top: 3px; width: 710px;">'
									...
									+ '<input type="hidden" id="fullName" value="'+obj.StudentDTO.fullName+'"><input type="hidden" id="classId" value="'+obj.StudentDTO.classId+'"><input type="hidden" id="sectionId" value="'+obj.StudentDTO.sectionId+'"></div><div class="row-fluid"  style="border: 1px solid lightgray; height: 113px; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;"><div class="span6">';
...
+'style="width: 127px; height: 113px;">'
							...
							$
									.each(
											...
											function(key, value) {
												...
												if (value.transportFee == 0) {
													
												} else {
													var transport = value.transportFee;
												...
												var netAmount = parseInt(openingBalance)
														...
														+ parseInt(value.bounceFineAmount);
												...
												monthwithfeedetail += '<th width="50%" class="head blue" style="color: white">Payable Amount<ul class="buttons"></th><th width="50%" class="head blue" style="color: white">Action';
														...
														.each(
																...
																function(key,
																	...
																	if (value1.feeCategoryId == 14
																		...
																		if (value1.feeCategoryName != null) {
																			monthwithfeedetail += value1.feeCategoryName
																					+ '</td>';
																			monthwithfeedetail += '<td width="10%">'
																					+ value1.amounts+'</td><td><input type="checkbox" checked="true" style="width: 16px; height: 14px;" id="ID'+count+'" onclick="addRemoveFee('+value1.amounts+','+count+')"></td>'
																		}
																	}
																	monthwithfeedetail += '</tr>';
																});
							...
							payment += '<tbody><tr><td style="width: 30px;">Payable Transport :</td><td style="width: 50px;">';
							payment += '<input type="text" readonly="readonly" style="height: 23px;" value="0" name="feeSubmissionDetailsDTOs[0].paybleTransport" id="paybleTransport"></td>';
							payment += '<td style="width: 30px;">Paid Transport :</td> <td style="width: 50px;">';
							payment += '<input type="text" style="height: 23px;" id="paidTransport" value="0" name="feeSubmissionDetailsDTOs[0].paidTransport" onkeyup="getPaidTransport(this.value)"></td>';
							payment += '<td style="width: 70px;">Transport Due :';
							payment += ' </td><td style="width: 50px;">';
							payment += ' <input type="text" name="feeSubmissionDetailsDTOs[0].transportDue"  readonly="readonly" style="height: 23px;" value="0"  id="transportDue">';
							payment += ' </td></tr>'
							payment += '<tr><td style="width: 30px;">Gross Amount:</td><td style="width: 50px;">';
							...
							payment += ' </td></tr>'
							payment += '<tr><td style="width: 30px;">Net Payable Amt</td><td style="width: 50px;">';
							...
							payment += '</tr><tr><td>Amount in words</td><td colspan="1"><input type="text" readonly="readonly" style="height: 23px;" id="paidAmountInWord" value="" name="feeSubmissionDetailsDTOs[0].paidAmountInWord"></td>';
							payment += '<td style="width: 30px;">Bounce Fine:</td><td><input type="text" style="height: 23px;" readonly="readonly" name="feeSubmissionDetailsDTOs[0].bounceFineAmount" id="bounceFineAmount"></td><td><div id="transportId" style="display: none;"><a class="btn" href=javascript:submitTransport("'+id+'")>Transport</a></div><div id="addfee" style="display: none;">&nbsp;&nbsp; <a id="addmorebutton" class="btn" href="javascript:addmore();">Add More</a></div></div></td></tr></tbody></table></div></div>';

							$("#payment").append(payment);
							$("#monthwithfeedetail").append(monthwithfeedetail);
							$("#paybleTransport").val($("#payableTransportId").val());
	...
	function submitTransport(id,amount){
	
		 amount=$("#paidTransport").val();
		 var payabletransport=$("#paybleTransport").val();
		...
		$.ajax({	
			...
			data : {
				'id' : id,
				'amount' : amount,
				'payabletransport' :payabletransport
			},
			success : function(data, status) {
				
				alert("Trnasport Submitted Successfully !")
	...
	function getPaidTransport(paidTransport) {
		var paybleTransport = $("#paybleTransport").val();
		if (parseInt(paybleTransport) < parseInt(paidTransport)) {

			$("#paidTransport").val('');
		} else {

			var transportDue = $("#transportDue").val();
			transportDue = parseInt(paybleTransport) - parseInt(paidTransport);
			$("#transportDue").val(transportDue)
			$("#transportId").show();
	...
	function showChild(value, checked, grossAmount, fineAmount, dueAmount,
			bounceFineAmount) {
		$("#paybleTransport").val($("#payableTransportId").val());
		if ($("#payableTransportId").val() == 0) {
			$("#paidTransport").prop('disabled', true);
		}
		if (dueAmount == 0) {
		...
		if (checked) {
			...
			$("#discountAmount").val(
					parseInt(grossAmount + fineAmount) - parseInt(disAmount)
							+ parseInt(bounceFineAmount));
			$("#bounceFineAmount").val(parseInt(bounceFineAmount));
			if ($("#recentPaidAmount").val() == '') {
...
+ '].discount">'
			...
			$("#totalbounceFineAmount").val(
					parseInt($("#bounceFineAmount").val()));

			$("#totalTransportAmount").val($("#paybleTransport").val());
			$("#totalRecentTransportPaidAmount").val($("#paidTransport").val())
			$("#totalTransportDue").val($("#transportDue").val());
...
<div class="content">
	...
	<f:form commandName="studentFeeSubmissionDetailsDTO" method="POST"
		id="validateform" action="fee-payment" target="_blank">
...
<div class="separator"></div>
						...
						<div class="row-form">
								...
								<div class="span8">
									<input type="text" name="studentname"
										oninput="getStudentDatabyName(this.value);">
								</div>
							</div>
							<div class="row-form">
								<div class="span4">Adm. No:</div>
								<div class="span8">
									<input type="text" name="studentId"
										oninput="getStudentDatabyId(this.value);">
								</div>
								...
								<tbody>
									...
									<tr>
										...
										<td style="width: 50px;"><input type="text" id="totalDue"
											value="0" name="dueAmount" readonly="readonly"
											style="height: 23px;"></td>
									</tr>
									<tr>
										<td style="width: 30px;">Net Transport Amt</td>
										<td style="width: 50px;"><input type="text" value="0"
											name="netPayableTransportAmount" id="totalTransportAmount"
											readonly="readonly" style="height: 23px;"></td>
										<td>Paid Transport Amt.</td>
										<td><input type="text" name="recentPaidTransportAmount"
											readonly="readonly" value="0"
											id="totalRecentTransportPaidAmount" style="height: 23px;"></td>
										<td style="width: 30px;"><font color="red">Transport
												Due </font>:</td>
										<td style="width: 50px;"><input type="text"
											id="totalTransportDue" value="0" name="dueTransportAmount"
											readonly="readonly" style="height: 23px;"></td>
									</tr>
									...
									<tr>
										<td style="width: 30px;">Net Bounce Amt</td>
										<td style="width: 50px;"><input type="text"
											id="totalbounceFineAmount" readonly="readonly"
											style="height: 23px;" name="bounceFine"></td>
										<td></td>
										<td></td>
										<td></td>
	...
	<input type="hidden" id="refresh" value="no"> <input
		type="hidden" id="payableTransportId" value="0">
		<div id="transportSubmit" style="display: none"></div>
