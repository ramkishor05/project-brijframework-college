...
function getunpaiddues() {
	...
	var totalPaid=0;
	var totalDues=0;
		var url = 'getunpaiddues/' +classId+'/'+sessionId;
		$
				.ajax({
					url : url,
					...
					if(length==0)
						...
						+'<table cellpadding="0" cellspacing="0" width="100%" class="table"><thead><tr><th>Date</th><th>Student Name</th><th>Father Name</th>'
								+ '<th>Section</th><th>Payment Mode</th><th>Total Amount</th><th>Discount</th><th>Paid Amount</th><th>Dues</th><th>Mobile no.</th><th>Payment Status</th><th>Show</th><th>Pay Dues</th><th>Due Payment Details</th></tr></thead><tbody>';
						for ( var i = 0; i < length; i++) {
			...
			for ( var i = 0; i < length; i++) {
				if (j[i].paidStatus == 'InHand') {
					...
					content += '<td style="background-color: lightgray;"><select onchange="changePaidDueStatus(this.value,'+receiptno+','
							...
							+ ')"style="width: 94px;"><option value="Inprogress">'
							+ j[i].paidStatus
							+ '</option><option value="Completed">Completed</option><option value="Cancel"><span style="color:red">Cancel</span></option></select></td><td><a href="getDuesReceipt?id='
							+ j[i].receiptno
				...
				if (j[i].paidStatus == 'Cancel') {
					...
					content += '<td style="background-color: pink;">'
							...
							+ j[i].dues
							+ '</td><td style="background-color: pink;">'
							+ j[i].paidStatus + '</td><td><a href="getDuesReceipt?id='
							+ j[i].receiptno
							+ '" target="_blank"><span class="label label-success">Receipt</span></a></td></tr>';
...
function paydues(receiptno,dues)
{
	$("#pays").empty();
			var st = '<div id="finals"><div class="block"><div class="head blue" style="height:30px;padding:0.5px 0 0 20px;margin-top: 0px;"><h4>Student Due Details</h4><a href="javascript:hidepaybox();" class="close" data-dismiss="modal"'
				+'style="margin-right: 10px; margin-top: -37px;" aria-hidden="true"><span class="ico-remove"></span></a></div><form action="dues-payment" method="POST" commandName="DressPaymentDTO" target="_blank" id="validateform"><table style="width: 1006px; padding-left: 54px; margin-left: 28px; margin-top: 11px;"><tr><td style="width: 136px;">Total amount:</td><td style="width: 90px;"> <input type="text" name="totalAmount" id="totalAmount" readonly="readonly" value='+dues+'></td><td style="width: 28px;"></td>'
...
function validate() {
	...
	if (flage) {
		$("#feeCategoryNamemsg").empty();
		$("#validateform").submit();
		document.location.href = 'pay-uniform-dues';
	}
...
<div class="content">
	<div id="content-header" style="height: 150px;">
		...
		<h1>Pay Dues of Uniform</h1>
		<h3>Clear dues of sold uniform</h3>
