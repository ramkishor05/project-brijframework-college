...
<script>
	function showpayment(value, paymentMode, chequeno, bankName,paidStatus,date,amount) {
		...
		if (paymentMode == 'Cheque') {
		 ...
		 +paidStatus
		 + '</td></tr>'
		 + '<tr><td>Cheque No.</td><td class="center">'
		 + chequeno
		 + '</td></tr>'
		 + '<tr><td>Bank Name</td><td class="center">'
		 + bankName + '</td></tr>'
		...
		$
				.ajax({
					...
					success : function(data, status) {
						...
						var detail = '<div style="padding: 10px 0 0;" class="widget-body"><div class="block" class="block" style="width: 493px;"><div class="head green" style="height: 15px; width: 480px;">'
								...
								+ '<th style="background-color: #FFCC99;  padding-left: 18px;"">Gross Amount</th>'
								+ '<th style="background-color: #FFCC99">Discount</th>'
								+ '<th style="background-color: #FFCC99">Paid Amount</th>'
								+ '<th style="background-color: #FFCC99">Balance Amount</th>'
								+ '<th style="background-color: #FFCC99">Uniform Details</th>'
								+ '<th style="background-color: #FFCC99">Payment Mode</th>'
						...
						for (i = 0; i < j.length; i++) {
							...
							detail += '<tr>'
									+ '<td style="background-color: #FFFFCC; width: 60px;">'
									+ j[i].dateOfPurchase
									+ '</td>'
									+ '<td style="background-color: #FFFFCC;width: 52px;">'
									+ j[i].grossAmount
									+ '</td>'
									+ '<td style="background-color: #FFFFCC; width: 52px;">'
									+ j[i].discount
									+ '</td>'
									+ '<td style="background-color: #FFFFCC; width: 50px;">'
									+ j[i].paidAmount
									+ '</td>'
									+ '<td style="background-color: #FFFFCC; width: 45px;">'
									+ j[i].balanceAmount
									+ '</td>';
		...
		$
				.ajax({
					...
					success : function(data, status) {
						...
						var j = JSON.parse(data);
						var content = '<table width="50%" cellspacing="0" cellpadding="0" class="table"style="width: 174px; border:1px solid gray;"><thead><tr><th colspan="6" class="head red" class="head blue"  style="color: white; padding-left: 17px;">Uniform Details'
								+ '<ul class="buttons"><li><a href="javascript:hideuniform('
								+ id
								+ ');"><div class="icon"><span class="ico-cancel"></span></div></a></li></ul></th> </tr></thead><tbody><tr><td style="background-color: #FFFFFF" class="center">Name</td><td class="center" style="background-color: #FFFFFF">Category</td><td class="center" style="background-color: #FFFFFF">Size</td><td class="center" style="background-color: #FFFFFF">Quantity</td><td class="center" style="background-color: #FFFFFF">Amount</td><td class="center" style="background-color: #FFFFFF">Net Amount</td></tr>'
	...
	function payment() {
		...
		var st = '<div id="finals"><div id="box" style="border: 1px solid; border-radius: 10px; width: 717px; height: 161px; padding-left: 22px; margin-top: 0px; padding-top: 5px;"><table style="width: 1006px; padding-left: 54px;"><tr><td style="width: 136px;">Total amount:</td><td style="width: 90px;"> <input type="text" name="grossAmount" id="totalAmount" readonly="readonly" value='+total+'></td><td style="width: 28px;"></td>'
	...
	function validate() {
		...
		if (flage) {
			$("#feeCategoryNamemsg").empty();
			$("#validateform").submit();
			//document.location.href = 'uniform-purchase';
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						$("#edituniformName").val(obj.dressName);
						$("#edituniformCategory").val(obj.category);
						$("#edituniformSize").val(obj.size);
	...
	function editedvalidate() {
		...
		if (!regex1.test($("#editquantity").val())) {
			...
			$("#editshopmsg")
					.append(
							'<span><font color="red" size="2">* Net Amount should be greater than zero and Enter only numeric value in amount</font></span>');
			flage = false;
		} else if ($("#edituniformSize").val() == '') {
			$("#editsize")
					.append(
							'<span><font color="red" size="2">*Enter Size</font></span>');
			flage = false;
		} else if (!regex2.test($("#edituniformCategory").val())) {
			$("#editcategory")
					.append(
							'<span><font color="red" size="2">*Enter only letters</font></span>');
			flage = false;
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						$("#totals").val(t);
						var st = '<div id="tbox" style="border: 1px solid lightgray; width: 189px; padding-left: 14px; margin-bottom: 18px; float: left; background-color: lightyellow; margin-top: 63px;"><h5>Gross amount:Rs '
								+ t
								...
								+ '<div class="block"><div class="head blue" style="width: 519px;"><div class="icon"><span class="ico-pen-2"></span></div>'
								+ '<h2>Uniform Purchase Details</h2><ul class="buttons"><li><a return false; href="#"><div class="icon"><span class="ico-info"></span></div></a></li>'
								+ '</ul></div><div id="purchases"><table class="table fTable lcnp" style="width: 1059px;"><thead><tr><th>S No</th><th>Name</th><th>Category</th>'
								+ '<th>Size</th><th>Quantity</th><th>Price</th><th>Amount</th><th>Net Amount</th><th>Edit</th><th>Delete</th></tr></thead><tbody>';
						for (var i = 0; i < j.length; i++) {
...
<div class="paybox">
	<div class="paybox-header">
		...
		<h4 id="myModalLabel" style="padding-top: 15px;"><center>Payment details</center></h4>
	</div>
...
<div class="editbox">
	...
	<f:form commandName="AllotDressDTO" action="edit-uniform-allot.html"
		id="submitform" onsubmit="return editedvalidate();">
		<div class="row-fluid" style="margin-top: 20px;">
			<div class="block-fluid">
				<div id="appendallotDressId"></div>
				...
				<table>
					...
					<tr>
						...
						<td><f:input id="edituniformName" maxlength="20"
								required="required" path="dressName" style="width:200px;" /></td>
					...
					<tr>
						...
						<td><f:input id="edituniformCategory" maxlength="15"
								required="required" path="category" style="width:200px;" /></td>
						<td><div id="editcategory"></div></td>
					</tr>
					<tr>
						...
						<td><f:input id="edituniformSize" maxlength="10"
								required="required" path="size" style="width:200px;" /></td>
						<td><div id="editsize"></div></td>
					</tr>
					<tr>
						<td style="padding-left: 20px; padding-right: 21px;"><span>Quantity<font
								color="red">*</font> :
						</span></td>
						<td><f:input id="editquantity" maxlength="6"
								oninput="getEditAmountupdate(this.value);" required="required"
								path="quantity" style="width:200px;" /></td>
...
<div class="content">
	<div id="content-header" style="height: 150px;">
		...
		<h3>Payment of purchased Uniforms</h3>
	...
	<br>
	<f:form action="pay-purchases" id="validateform"
		commandName="UniformSupplierDTO" target="_blank">
	...
	<br>
		<div id="pays" style="margin-top: 29px; margin-bottom: -101px; width: 741px;"></div>
	</f:form>
	<c:if test="${msg!=null}">
	...
	<div id="whole">
		<div>
			...
			<div id="supplierDetails"  style="float: right; margin-top: 80px; width: 493px;"></div>
			<div id="detailofdress" style="float:left;">
				<c:if test="${!purchaseList.isEmpty()}">
					<div
						 id="tbox"  style="border: 1px solid lightgray; width: 189px; padding-left: 14px; margin-bottom: 18px; float: left; background-color: lightyellow; margin-top: 63px; ">
					...
					<div style="padding: 10px 0px 0px; width: 502px;"
						class="widget-body">
						<div class="block">
							<div class="head blue" style="width: 519px;">
								<div class="icon">
									...
									<tbody>
										<c:forEach var="list" items="${purchaseList}" varStatus="loop">
											<tr>
												...
												<td class="center"><font size="2">${list.dressName}</font></td>
												<td class="center"><font size="2">${list.category}</font></td>
												<td class="center"><font size="2">${list.size}</font></td>
